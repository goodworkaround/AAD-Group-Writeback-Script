...
Import-Module .\AuthenticationMethods\ClientCertificate.psm1 -Force -Verbose:$VerbosePreference
...
elseif ($Config.AuthenticationMethod -eq "ClientCertificate") {
    $AccessToken = Get-ClientCertificateMSGraphAccessToken -ClientID $Config.ClientID -Thumbprint $Config.Thumbprint -TenantID $Config.TenantID -LoginUrl $graphEndpoints.LoginUrl -GraphUrl $graphEndpoints.GraphUrl -Verbose:$VerbosePreference
}
else {
